        ...
        <tr>
          <td>
            ...
            <div v-if="a.result.fulfillment.speech" class="bubble bot">
              <p v-if="a.result.fulfillment.speech == ReadPickForUser" >
             {{ repeat() }} </p>
              <p2
                v-else="a.result.fulfillment.speech == ReadPickForUser"
              >{{ a.result.fulfillment.speech }} </p2>
            </div>
...
export default {
  ...
  methods: {
    repeat: function() {
      this.setChatbotInputs = false;

      this.Return = "\n" + "Do you  want another one ?";
      return this.Joke + "         :) " ;
    },
    setChatbotInputs: function(verify) {
      this.ChatbotInput = verify;
    },
    async getJokeF() {
      let configJoke = {
        headers: {
